...
export default async function SavedPropertiesPage() {
  ...
  const savedProperties =
    (savedRecords?.map((record) => record.properties).filter(Boolean) as any[]) || []
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 md:px-8">
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-2">Properties you've bookmarked for later.</p>
          </div>
        </div>
        ...
        {savedProperties.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed py-32 text-center">
            ...
            <p className="text-muted-foreground mt-2 max-w-md">
              Start exploring campus housing and click the heart icon or "Save Property" button to
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            ...
            {savedProperties.map((property) => (
              <div
                key={property.id}
                className="group bg-card overflow-hidden rounded-2xl border transition-all hover:-translate-y-1 hover:shadow-lg"
              >
                <Link href={`/properties/${property.id}`} className="block">
                  ...
                  <div className="p-4">
                    <h3 className="text-foreground line-clamp-1 font-bold">{property.title}</h3>
                    <p className="text-muted-foreground mt-1 line-clamp-1 text-sm">
                      {property.address}
                    </p>
                    <div className="text-muted-foreground mt-3 flex items-center gap-3 text-sm font-medium">
