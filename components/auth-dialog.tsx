...
export function AuthDialog({ open, onOpenChange, defaultView = 'login' }: AuthDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="border-border bg-card overflow-hidden p-0 sm:max-w-[425px]">
        <DialogTitle className="sr-only">
          {view === 'login' ? 'Sign in to CampusNest' : 'Create a CampusNest account'}
        </DialogTitle>
        <DialogDescription className="sr-only">
          Authenticate to access your account or create a new one.
        </DialogDescription>
        ...
        <div className="flex flex-col items-center justify-center p-6 sm:p-8">
          <div className="w-full space-y-6">
            ...
            <form onSubmit={view === 'login' ? handleLogin : handleSignUp} className="space-y-4">
                      ...
                      <button
                        ...
                        className={`flex items-center justify-center gap-2 rounded-xl border py-2.5 text-sm font-semibold transition-all ${
                          userType === val
                            ...
                            : 'bg-muted/40 text-muted-foreground border-border hover:text-foreground hover:border-amber-400'
                        }`}
                      >
              ...
              <Button
                type="submit"
                className="w-full rounded-xl bg-amber-500 font-semibold text-white hover:bg-amber-600"
                disabled={isLoading}
              >
                {isLoading ? (
                  <>
