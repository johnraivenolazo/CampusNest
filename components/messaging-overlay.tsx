...
export default function MessagingOverlay({ user }: { user: User | null }) {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      ...
      setMessages((data || []) as OverlayMessage[])
  ...
  return (
    <div className="pointer-events-none fixed right-0 bottom-0 z-100 flex max-w-[calc(100vw-1rem)] flex-col items-end p-4 sm:max-w-none">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            animate={{
              ...
              width: isMinimized
                ? 'min(280px, calc(100vw - 1rem))'
                : 'min(400px, calc(100vw - 1rem))',
            }}
            exit={{ y: 20, opacity: 0, scale: 0.95 }}
            transition={{ type: 'spring', damping: 25, stiffness: 300 }}
            className="bg-background pointer-events-auto mb-4 flex flex-col overflow-hidden rounded-2xl border shadow-2xl"
          >
            {/* Overlay Header */}
            <div
              className={`flex shrink-0 items-center justify-between bg-amber-500 px-4 py-3 text-white dark:text-amber-950 ${isMinimized ? 'h-full' : ''}`}
            >
      ...
      {!isOpen && (
        <Button
          ...
          className="group pointer-events-auto relative h-14 w-14 rounded-full bg-amber-500 shadow-xl hover:bg-amber-600"
        >
